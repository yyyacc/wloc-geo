...
- 当前脚本按 `field5 = 海拔（米）` 写入，并将数值四舍五入为整数米。

如果输入 `5m` 却得到约 `500m`，通常表示代理软件仍在执行带 `altitudeScale=100` 的旧脚本，或仍启用了原版模块。请停用旧模块、重新导入当前模块并刷新脚本缓存。模块中的版本查询参数用于规避常见缓存，但不同代理软件的缓存策略可能不同，必要时请在软件内手动清除脚本缓存。
