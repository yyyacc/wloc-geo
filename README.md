...
| 变量名 | 示例值 | 作用 |
|------|------|------|
| `ALTITUDE_SCALE` | `100` | 可选：预填网页“海拔缩放系数” |

未配置时输入框保持为空，脚本内部仍按 `100` 处理；配置后会把变量值预填到输入框。该变量只决定网页初始值；要让真机脚本使用新值，需要打开网页并再次点击“储存到设备”。如果设备已有保存值，网页会显示设备当前值，可手动修改后重新保存。
...
3. 写入规则：`field5 = 海拔(米) × 海拔缩放系数`。
   - 默认缩放系数为 `100`，即米转换为厘米。
   - 网页输入框默认留空，留空时使用内置系数 `100`，且不额外保存覆盖值。
   - 手动填写的“海拔缩放系数”会随坐标一起保存到 `wloc_settings_v2`。
