...
- 真机测试确认 `field5` 使用厘米单位，设备读数还存在固定 `-1.58m` 偏移；当前按 `field5 = (目标海拔 + 1.58m) × 100` 写入补偿后的厘米值。
...
正常情况下，Worker 日志会显示 `[elevation] source=... altitude=5m`，代理日志会显示 `海拔=5m 补偿=+1.58m field5=658 patched=...`。前者用于确认地面海拔来源，后者用于确认补偿后的 WLOC 厘米值和成功修改的定位记录数。
